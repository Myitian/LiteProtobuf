...
{
    public sealed class GenericNonPackedClassList<T, THandler>
        ...
        where THandler : IClassProtobufTypeHandler<T>
    {
        ...
        public static bool TryCreateInstance(FieldInfo fieldInfo, SerializationOptions? options, [NotNullWhen(true)] out List<T>? value)
        {
            return Defaults.NewProtobufTypeFactory<List<T>>
                .TryCreateInstance<GenericNonPackedClassList<T, THandler>>(fieldInfo, options, out value);
        }
        public static List<T> CreateInstance(FieldInfo fieldInfo, SerializationOptions? options)
        {
            return Defaults.NewProtobufTypeFactory<List<T>>
                .CreateInstance<GenericNonPackedClassList<T, THandler>>(fieldInfo, options);
        }
            ...
            where TReader : struct, IStructBinaryReader<TReader>, allows ref struct
        {
            return Defaults.ReadOnlyClassProtobufTypeHandler<List<T>>
                .TryCreateFulfilled<GenericNonPackedClassList<T, THandler>, TReader>(ref reader, fieldInfo, options, out value, out status);
        }
        ...
        {
            return Defaults.ReadOnlyClassProtobufTypeHandler<List<T>>
                .TryCreateFulfilled<GenericNonPackedClassList<T, THandler>, TReader>(reader, fieldInfo, options, out value, out status);
        }
        public static List<T> CreateFulfilled<TReader>(scoped ref TReader reader, FieldInfo fieldInfo, SerializationOptions? options)
            where TReader : struct, IStructBinaryReader<TReader>, allows ref struct
        {
            return Defaults.ReadOnlyClassProtobufTypeHandler<List<T>>
                .CreateFulfilled<GenericNonPackedClassList<T, THandler>, TReader>(ref reader, fieldInfo, options);
        }
        public static List<T> CreateFulfilled<TReader>(TReader reader, FieldInfo fieldInfo, SerializationOptions? options)
             where TReader : class, IClassBinaryReader<TReader>
        {
            return Defaults.ReadOnlyClassProtobufTypeHandler<List<T>>
                .CreateFulfilled<GenericNonPackedClassList<T, THandler>, TReader>(reader, fieldInfo, options);
        }
            ...
            where TWriter : struct, IStructBinaryWriter<TWriter>, allows ref struct
        {
            foreach (T item in self)
                THandler.WriteProtobuf(item, ref writer, fieldInfo, options);
        }
        ...
        {
            foreach (T item in self)
                THandler.WriteProtobuf(item, writer, fieldInfo, options);
        }
    }
}